...
describe('calcDivisionForSingleAssignment', () => {
  ...
  const getHistoryWithEverythingForSimpleAssignment = () => {
    ...
    h.committeeSnapshotEvents.push({ block: 5, newRelativeWeightInCommittee: 0 });
    h.committeeSnapshotEvents.push({ block: 6, newRelativeWeightInCommittee: 0.25 });
    h.committeeSnapshotEvents.push({ block: 8, newRelativeWeightInCommittee: 0.5 });
    // delegations
  ...
  it('works on assigment index 0 blocks 1-5', () => {
    ...
    expect(division.amountsWithoutDelegate['D1']).toEqual(new BN(3375));
    expect(division.amountsWithoutDelegate['D2']).toEqual(new BN(1875));
    expect(division.amountsWithoutDelegate['D3']).toEqual(new BN(750));
  });
  ...
  it('works on assignment index 1 blocks 6-10', () => {
    ...
    expect(division.amountsWithoutDelegate['D1']).toEqual(new BN(4875));
    expect(division.amountsWithoutDelegate['D2']).toEqual(new BN(2125));
    expect(division.amountsWithoutDelegate['D3']).toEqual(new BN(5000));
  });
