// taken from (and tested in) orbs-network/management-service/src/ethereum/event-fetcher-lookahead.ts
...
export class LookaheadEventFetcher extends EventFetcher {
  ...
  constructor(eventName: EventName, reader: EthereumAdapter, autoscaleOptions?: Partial<AutoscaleOptions>) {
  ...
  protected async downloadNewPage(blockNumber: number, latestAllowedBlock: number) {
    ...
    while (!successful) {
      try {
        ...
        this.lookAhead = await this.reader.getPastEvents(
          this.eventName,
          { fromBlock, toBlock },
          this.contract,
          this.filter
